 ...
 * /////////////////------->  email: mohamedsaeed.mso11@gmail.com  <---------------\\\\\\\\\\\\\\\\\\\\\\\ */
 ...
 {
     return '<a href="'.$uri.'" '.$attributes.' class="btn btn-primary" >
 ...
 {
     return '<a onclick="return confirm(\' '.__('ms_lang.r_u_sure_delete').'\');" '.$attributes.' class="btn btn-danger">
 ...
 {
     return '<a onclick="return confirm(\' '.__('ms_lang.r_u_sure_undelete').'\');" '.$attributes.' class="btn btn-warning pl-1 pr-1">
 ...
 {
     return '<a onclick="return confirm(\' '.__('ms_lang.r_u_sure_fulldelete').'\');" '.$attributes.' class="btn btn-danger pl-1 pr-1">
 ...
 function add_btn_active($uri='',$attributes='',$status_v='Y',$act_w='')
 {
     ...
     return '<a href="'.$uri.'" '.$attributes.' class="btn btn-'.$styl.'" style="padding: 9px 0px;font-weight: bolder">'.$act_w.' </a>';
 }
 /// busy button
...
{
    if(substr($url_go, 0,7) == 'http://' || substr($url_go, 0,8) == 'https://')
    {
        $url_chekd=$url_go;
    }else
    {
        $url_chekd='https://'.$url_go;
    }
    return $url_chekd;
}
...
function send_sms_from_yamamah($reciever_number="569000711",$msg_send="رسالة اختبارية",$tag_name="kidzone")
{
    $sms_data = [
        "Username" => "",
        "Password" => "", //
