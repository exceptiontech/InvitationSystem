...
class PaymentsPlans extends Component
{
    ...
    public $showDeleted;
    public $btn_kwrd;
    public $edit_object,$title_page;
    protected $listeners=[
        'objectEdit'=>'refresh_edited'
    ];
    public function render()
    {
        $results=PaymentPlan::with('category')->get();
        return view('livewire.admin.payments_plans.payments_plans',compact('results'))->extends('admin.layouts.app');
    }
    ...
    public function edit_form($id=0)
    {
        ...
        $this->showForm=false;
        $edit_object = PaymentPlan::find($id);
    ...
    {
        $data= PaymentPlan::select('id','is_active')->find($id);
        if($data->is_active == 'Y')
        {
            $data->is_active='N';
        }
