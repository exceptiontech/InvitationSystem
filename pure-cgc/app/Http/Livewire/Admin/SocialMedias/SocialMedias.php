...
namespace App\Http\Livewire\Admin\SocialMedias;
...
class SocialMedias extends Component
{
    ...
    public function render()
    {
        ...
        $results= SocialMedia::get()->sortByDesc('ord');
        return view('livewire.admin.social-medias.social-medias',
                    [
                        'results'=>$results,
    ...
    public function edit_form($id=0)
    {
        ...
        if($id > 0)
        {
            ...
            $edit_object= SocialMedia::where('deleted_at',null)->whereId($id)->first();
        }
        else
        {
            if($this->showForm == true)
    ...
    {
        $data=SocialMedia::select('id','is_active')->find($id);
        if($data->is_active == 'Y')
        {
            $data->is_active='N';
        }
    ...
    {
        $data=SocialMedia::select('id','deleted_at')->find($id);
