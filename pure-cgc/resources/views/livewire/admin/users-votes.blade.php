          ...
          <table class="table table-bordered table-hover table-checkable" id="kt_datatable" style="margin-top: 13px !important">
            <thead>
              <tr>
                ...
                <th>{{ __('ms_lang.vote') }}</th>
                <th>{{ __('ms_lang.select_user_2win') }}</th>
...
?>
                <tr>
                    ...
                    <td>{{ Auth::user()->user_lang=='ar' ? $result->vote_point->title : $result->vote_point->title_en }}</td>
                    <td style="width: 11%;">
                        @if ($result->is_win == 'N')
                        <a href="javascript::void(0);" wire:click='win_user({{ $result->id }})'><button class="btn btn-default bg-warning text-white">{{ __('ms_lang.btn_select') }}</button></a>
                        @else
                            <a href="javascript::void(0);" wire:click='win_user({{ $result->id }})'><button class="btn btn-default bg-success text-white">{{ __('ms_lang.selected') }}</button></a>
                        @endif
