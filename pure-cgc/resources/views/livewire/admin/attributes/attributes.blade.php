...
@if ($showForm == true)
        <livewire:admin.attributes.edit />
@elseif ($showDeleted == true)
        <livewire:admin.attributes.deleted />
@else
...
@if ($result->parent_id == 0 )
                         {{-- <a href="{{ url(config('app.url_admin').'attributes/'.$result->id) }}"><button class="btn btn-default bg-info">{{ __('ms_lang.sub_category_t') }}</button></a> --}}
@endif
