...
<div class="card card-custom">
	<div class="card-header flex-wrap py-5">
		<div class="card-title">
			<h3 class="card-label">{{Auth::user()->user_lang=='ar' ? 'منتجات الخدمات ' :'product services'}}
...
@if ($showForm == true)
        <livewire:admin.category-services.create />
@elseif ($showFormEdit == true)
        <livewire:admin.category-services.edit />
@elseif ($showDeleted == true)
        <livewire:admin.category-services.deleted />
@else
    ...
    <table class="table table-bordered table-hover table-checkable" id="kt_datatable" style="margin-top: 13px !important">
        <thead>
          <tr>
            ...
            <th>{{ __('ms_lang.logo') }}</th>
            <th>{{ __('ms_lang.url_link_t') }}</th>
...
?>
            <tr>
                ...
                <td style="padding: 0px;">
                    <img src="{{ img_chk_exist($result->logo) }}" style="width: 70px; height: 60px" />
                  </td>
                <td>{{$result->url}}</td>
