      ...
      <div class="dataprofile">
        <div class="dataimg">
          ...
          <nav class="social">
            ...
            <a href="{{url_chk($result->facebook)}}" class="fab fa-facebook-f icon-facebook" title="Facebook"></a>
            @endif
            @if (!empty($result->instagram))
            <a  href="{{url_chk($result->instagram)}}" class="fab fa-instagram icon-instagram" title="Instagram"></a>
            @endif
            @if (!empty($result->twitter))
            <a href="{{url_chk($result->twitter)}}" class="fab fa-twitter icon-twitter" title="Twitter"></a>
            @endif
            @if (!empty($result->youtube))
            <a href="{{url_chk($result->youtube)}}" class="fab fa-youtube icon-youtube" title="youtube"></a>
            @endif
          </nav>
        </div>
        <div class="dat-user">
          ...
          <div class="datatype">
            <div class="row">
              <div class="col-sm-4 childbox">
                <div class="rowdata">
                  <p>Country</p><span>{{$result->country_of_residence_name}}     </span>
                </div>
              </div>
              <div class="col-sm-5 childbox">
                <div class="rowdata">
                  <p> City</p><span> {{$result->city_of_residence_name}}    </span>
                </div>
              </div>
              <div class="col-sm-3 childbox">
                ...
                {{-- <a class="view" href="{{route('members.show',[$result->id,'n'=>title_2url($result->name)])}}" target="_blank">View profile as someone</a> --}}
              </div>
            </div>
          </div>
        </div>
...
@endif
      ...
      <div class="resultsbox recomen">
        <h2 class="titlebold"> My Branches </h2>
@if (count($gt_branches))
    @foreach ($gt_branches as $gt_branche)
        <div class="itmeuser">
          <div class="imguser"><img src="{{img_chk_exist($gt_branche->img)}}" alt=""></div>
          <div class="usertext">
            <h3>{{$gt_branche->name}} </h3><a class="costarica" >{{$gt_branche->country_name.'/'.$gt_branche->city_name}}</a>
            <p>{{$gt_branche->bio}} </p>
            
            <div class="botright"><a></a><a class="bottom" href="{{route('members.show',[$gt_branche->id,'n'=>title_2url($gt_branche->name)])}}" target="_blank" >View profile </a></div>
          </div>
        </div>
    @endforeach
@else
        <h1><center>No Branches</center></h1>
@endif
